...
# parse command line
try:
    infilename, outbasename = sys.argv[1:]
except ValueError: # wrong number of values to unpack
    sys.stdout.write("Error: Expected %i, got %i arguments.\n" % (len(sys.argv) - 1))
    sys.stdout.write("Syntax: %s [input file] [output basename]\n" % (sys.argv[0]))

# parse input file
json_doc = open(infilename).readlines()
json_doc = "".join([x for x in json_doc if not x.startswith('#')])
json_def = json.JSONDecoder().decode(json_doc)

def autogenWarning(file):
...
# begin include guard
guardDefine = outbasename.upper().replace("-", "_").replace("/", "_") + "_H"
hf.write("#ifndef " + guardDefine + "\n")
hf.write("#define " + guardDefine + "\n")
